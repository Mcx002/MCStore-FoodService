...
describe('Utils Env Parser Test', () => {
    ...
    test('getEnvNumber() Should return error that env variable is not set yet', () => {
        expect(() => getEnvNumber('TEST2', undefined)).toThrow(
            'TEST2 is not set yet'
        )
    })
    ...
    test('validateDefaultValue Should throw variableName is not set yet', () => {
        ...
        expect(() => validateDefaultValue(variableName, undefined)).toThrow(
            `${variableName} is not set yet`
        )
    })
    ...
    test('Should getEnvBoolean throw error', () => {
        expect(() => getEnvBoolean('BOOL_ENV_NONE')).toThrow(
            'BOOL_ENV_NONE is not set yet'
        )
    })
})
